  ...
  display() {
    ...
    if (this.index % 2 === 0) {
      nuevaAltura = map(
        ...
        this.altura,
        this.altura + 80
      ); // EXPANDE la altura de las filas pares 25,100
      //nuevaPosY = map(gestorAmp.filtrada, 0, 1, this.y, 100);
    } else {
      nuevaAltura = map(
        1 - gestorAmp.filtrada,
        0,
        1,
        this.altura - 20,
        this.altura
      ); // CONTRAE la altura de las filas impares 125,200
...
class Celda {
  ...
  display() {
    let c = color(
      this.tinte,
      map(sin((frameCount * 0.1 + this.x / this.ancho) * 0.4), -1, 3, 0, 255),
      map(gestorPitch.filtrada, 0, 1, 0, this.brilloOriginal)
    );
